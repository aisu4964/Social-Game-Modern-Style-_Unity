...
public class MyPageIconJumpingAnimation : MonoBehaviour
{
    ...
    [Header("選択しているシーンの名前")] public string ExecutionSceneName = "MyPage";
    #endregion
    ...
    void OnEnable()
    {
        if (SceneManager.GetActiveScene().name == ExecutionSceneName) //もし現在のシーン名が、[ExecutionSceneName]変数内のシーン名と同じ場合のみアニメーションを開始する
        {
    ...
    void OnDisable()
    {
        transform.DOKill(); //DOTweenアニメーションを即座に停止する
        transform.position = originalPosition; //現在の座標にoriginalPosition変数の数値を代入する
    }
    ...
    void ChangeImage() //新しい画像に差し替え、大きさ位置を変更するメソッド
    {
        ...
        if (rectTransform != null) //もしrectTransformの中身がnullではない場合は下記を実行する
        {
            ...
            float newWidth = 145; //少数点以下が入る箱に150を代入した
