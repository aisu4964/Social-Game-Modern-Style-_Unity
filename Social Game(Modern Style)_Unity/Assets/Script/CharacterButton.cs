...
public class CharacterButton : MonoBehaviour, IPointerClickHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    [Header("画像の色を変えるシーン名")] public string ColorSceneName = "Character";
    ...
    {
        onClickCallback = GoToCharacter; //onClickCallbackにGoToCharacterメソッドを代入した
    ...
    public void OnPointerDown(PointerEventData eventData) //ボタンを長押ししている最中に実行
    {
        ...
        {
            image.DOColor(new Color(0.75f, 0.75f, 1), 0); //image内の画像を0秒で色を変更
        }
    }
    ...
    #region//メソッド
    public void GoToCharacter() //Characterのシーンに移行するためのメソッド
    {
        _canvasGroup.interactable = false; //CanvasGroupコンポーネントのInteractableプロパティを無効にする(タッチ入力できなくなる)
        StartCoroutine(LoadSceneWithTransition("Character")); //Characterのシーンに移行するためのアニメーションとシーン移行を行うコルーチンを起動
    }
    ...
    void ChangeColor() //画像の色を赤に変えるメソッド
    {
        ...
        {
            image.DOColor(new Color(0.75f, 0.75f, 1), 0); //image変数内の画像の色を赤色に変える
        }
    }
    #endregion
}
